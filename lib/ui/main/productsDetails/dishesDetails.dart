...
class DishesDetailsScreen extends StatefulWidget {
  static const routeName = '/dishesdetails';
  ProductInfoModel productInfo;
  DishesDetailsScreen({Key key,this.productInfo}) : super(key: key);
  @override
...
class _DishesDetailsScreenState extends State<DishesDetailsScreen> with SingleTickerProviderStateMixin {
  ...
  refreshData(bool status,String message){
       ...
       setState(() {
         _this.isWishlist = !_this.isWishlist;
       });
  }
  _onFavoriteClick(){

    account.manageFavouriteItem(_this, refreshData);




    //account.revertFavoriteState(body,_this.isWishlist,);
    /*account.revertFavoriteState(body,_this.isWishlist,);
    setState(() {
    });*/
  ...
  _onPress(int count){
    dprint("Count = $count");
   // _this.count = count;
    //_this.productPrice[0].cartQty = count.toString();
    finalPrice = (double.parse(_this.productPrice[0].sp)  * count) as double;
    setState(() {});
  }

  _tapAddToCart(){
    print("User pressed \"Add to cart\" button. Count = ${_this.productPrice[0].cartQty}");
    /*if (!account.isAuth())
  ...
  /*DishesData _this;*/
  ProductInfoModel _this;

  @override
  void initState() {
    _this = widget.productInfo;
    if(_this.productPrice[0].cartQty =="0"){
      _this.productPrice[0].cartQty="1";
    }
    finalPrice = double.parse(_this.productPrice[0].cartQty) * double.parse(_this.productPrice[0].sp);
    ...
    imageFiles = json.decode(_this.mImages);
    if(imageFiles is List){
      _tabController = TabController(vsync: this, length: imageFiles.length+1);
                          ...
                          child: Row(
                            ...
                            Flexible(
                              /*child: FittedBox(child: Text(basket.makePriceString(basket.getItemPrice(_this)),*/
                                child: FittedBox(child: Text( finalPrice.toString(),
                                style: theme.text20boldWhite,),
                              )),
                              Container(
                                padding: EdgeInsets.only(left: 10, right: 10),
                                child: IButtonCount(textStyle: theme.text20boldWhite, color: Colors.white, count: int.parse(_this.productPrice[0].cartQty)??1 , pressButton: _onPress,),
                              )
                            ],
                          )
                      )
  ...
  _children(){
    ...
    var _dPrice = "";
    var _price = "";
    if (_this.productPrice[0].originalprice != null)
      _price = _this.productPrice[0].originalprice;
    if (_this.productPrice[0].sp != 0) {
      _price = basket.makePriceString(double.parse(_this.productPrice[0].sp) );
      _dPrice = _this.productPrice[0].originalprice;
    }

    var _addedName = "";
    /*if (_this.productName != null)
    ...
    list.add(Container(
      ...
      child: Row(
        children: [
          Expanded(
            child: IList1(imageAsset: (theme.vendor) ? "assets/orders2.png" : "assets/orders.png", text: "${_this.productName} $_addedName",                          // dish name
    ...
    if (_this.description.isNotEmpty) {
      list.add(Container(
        margin: EdgeInsets.only(left: 10, right: 10),
        child: Text(_this.description, style: theme.text14),                                       // dish description
      ));
      list.add(SizedBox(height: 20,));
    }
  ...
  _imageBuild(){
    ...
    listContain.add(Hero(tag: idHeroes,
        child: cacheImageWidgetContain("$serverImages${_this.productImage}", theme.colorPrimary)));
    list.add(Hero(tag: idHeroes,
        child: cacheImageWidgetCover("$serverImages${_this.productImage}", theme.colorPrimary)));
      ...
      children: [
        ...
        if (_this.productPrice[0].sp != 0)
          Container(
            ...
            child: Humorist(child: saleSticker(windowWidth*0.5, "-"+_this.productPrice[0].discountPercentage.toString()+"%", basket.makePriceString(double.parse(_this.productPrice[0].sp)), basket.makePriceString( double.parse(_this.productPrice[0].originalprice)))),
          ),
        if (account.isAuth())(
            Container(
              ...
              child: Stack(
                children: <Widget>[
                  Container(
                      ...
                      height: 30,
                      alignment: Alignment.center,
                      child: Icon(_this.isWishlist ? Icons.favorite : Icons.favorite_border, color: theme.colorPrimary, size: 30,)
                  ),
                  Positioned.fill(
                    child: Material(
