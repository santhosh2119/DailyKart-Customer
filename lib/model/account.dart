...
class Account {
  ...
  manageFavouriteItem(ProductInfoModel productInfoModel,Function refreshData){
    if(productInfoModel.isWishlist){
      print("Start removing product from wishlist");
      removeFromFavouriteItem(productInfoModel.id,refreshData);
    }else{
      print("Start adding product to wishlist");
      addItemToFavouriteList(productInfoModel.id,refreshData);
    }

  }

  addItemToFavouriteList(String id,Function refreshData){
    var body =
    {
    ...
    NetworkUtils.isNetwork().then((intenet) {
      if (intenet != null && intenet) {
        ...
        CommonMethods.getApiRepository().addItemToFavourite(body).then((response) {
          Fimber.d("addToFavoritesState updated Success");
          if (response != null) {
            print("Response : $response");
            refreshData(true,"success");
          }
        },
          onError: (exception) {
            refreshData(false,"failed");
          },
        );
  ...
  removeFromFavouriteItem(String id,Function refreshData){
    var body =
    {
      'user_id': '${PreferenceHelper.getString(AppConstants.PREF_USER_ID)}',
      'product_id':'${id}'
    ...
    NetworkUtils.isNetwork().then((intenet) {
      if (intenet != null && intenet) {
        ...
        CommonMethods.getApiRepository().removeFromFavouriteItems(body).then((response) {
          Fimber.d("removeFromFavouriteItem updated Success");
          if (response != null) {
            print("Response : $response");
            if(response.status){
              refreshData(true,"success");
            }else{
              refreshData(false,response.message);
            }
